...
import { getAdminHubCards } from '../../utils/adminHub';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';

export default function AdminHubPage() {
  const { user } = useAuth();
  const cards = getAdminHubCards(user);

  if (!cards.length) {
    return (
      <PageShell title="Administration" subtitle="Accès réservé à la direction.">
        <Card>
          <CardHeader>
            <CardTitle>Accès refusé</CardTitle>
            <CardDescription>Vous n&apos;avez pas accès à l&apos;administration.</CardDescription>
          </CardHeader>
        </Card>
      </PageShell>
    );
  ...
  return (
    <PageShell
      ...
      subtitle="Raccourcis vers la configuration et la gestion de l'établissement."
    >
